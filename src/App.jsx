...
const MOODS = {
  sad:     { label:'melancholy',  oh:'#b8c8e8', oc:'#2a4a7a', od:'#050d1f', gs1:'rgba(40,80,160,0.45)',   gs2:'rgba(20,50,120,0.22)',  ringCol:['#b8d0ff','#8ab0e8','#c8d8f0','#a0c0f8','#dde8ff'], faerieCol:['#b8d0ff','#8ab0e8','#a8c8ff','#c0d8ff','#ddeeff'] },
  happy:   { label:'joyful',     oh:'#fff4a0', oc:'#e8a020', od:'#3d1a00', gs1:'rgba(255,200,60,0.55)',  gs2:'rgba(230,140,20,0.28)', ringCol:['#fff4a0','#ffd060','#ffe880','#ffb830','#ffffff'],  faerieCol:['#fff4a0','#ffd060','#ffe880','#ffb830','#ffeebb'] },
  love:    { label:'loving',     oh:'#ffd0e8', oc:'#c0306a', od:'#400020', gs1:'rgba(220,80,140,0.5)',   gs2:'rgba(180,40,100,0.25)', ringCol:['#ffd0e8','#ffaad0','#ff80b8','#ffc0e0','#ffffff'],  faerieCol:['#ffd0e8','#ffaad0','#ffb8d8','#ffc8e4','#fff0f8'] },
  mystery: { label:'mysterious', oh:'#a0f0f0', oc:'#086878', od:'#020e12', gs1:'rgba(20,160,180,0.45)',  gs2:'rgba(10,100,130,0.22)', ringCol:['#a0f0f0','#60d0d8','#80e8f0','#40c0d0','#c0f8ff'],  faerieCol:['#a0f0f0','#60d0d8','#80e8f0','#b0f4ff','#e0fdff'] },
  angry:   { label:'fierce',     oh:'#ffb090', oc:'#b82010', od:'#250000', gs1:'rgba(220,60,20,0.55)',   gs2:'rgba(180,30,10,0.28)',  ringCol:['#ffb090','#ff7050','#ff9070','#ff5030','#ffd0c0'],  faerieCol:['#ffb090','#ff7050','#ff9070','#ffc0a0','#ffd8c8'] },
  nature:  { label:'natural',    oh:'#b8f0b0', oc:'#1a7a2a', od:'#021008', gs1:'rgba(40,160,60,0.45)',   gs2:'rgba(20,120,40,0.22)',  ringCol:['#b8f0b0','#80d880','#a0e8a0','#60c860','#d8f8d0'],  faerieCol:['#b8f0b0','#80d880','#a0e8a0','#c8f0c0','#e0f8e0'] },
  tech:    { label:'analytical', oh:'#a0c8ff', oc:'#1838c8', od:'#020818', gs1:'rgba(40,80,220,0.45)',   gs2:'rgba(20,50,180,0.22)',  ringCol:['#a0c8ff','#60a0ff','#80b8ff','#40a0ff','#d0e8ff'],  faerieCol:['#a0c8ff','#60a0ff','#80b8ff','#b0d0ff','#d0e8ff'] },
  wisdom:  { label:'wise',       oh:'#f0d890', oc:'#a06010', od:'#201000', gs1:'rgba(200,140,30,0.45)',  gs2:'rgba(160,100,20,0.22)', ringCol:['#f0d890','#e0b840','#f0c860','#d0a020','#fff0b0'],  faerieCol:['#f0d890','#e0b840','#f0c860','#f8e0a0','#fff8d0'] },
  neutral: { label:'awaiting',   oh:'#e8e0ff', oc:'#8b6ef5', od:'#2d1d7a', gs1:'rgba(120,90,255,0.3)',   gs2:'rgba(90,60,200,0.15)',  ringCol:['#ffffff','#e0d7ff','#c4b5fd','#a78bfa','#fde68a'],  faerieCol:['#e0d7ff','#c4b5fd','#fbcfe8','#fde68a','#bfdbfe','#d9f99d','#ffffff','#fecdd3','#a5f3fc','#f0abfc'] },
}

function detectMood(t) {
  t = t.toLowerCase()
  if (/\b(sad|grief|cry|miss|lost|lonely|heartbreak|depress|mourn|sorrow|hurt|pain|disappoint)\b/.test(t)) return 'sad'
  if (/\b(happy|joy|excit|celebrat|wonderful|amazing|great|fantastic|beautiful|blessed|thrill)\b/.test(t)) return 'happy'
  if (/\b(love|romance|crush|partner|relation|heart|together|marriage|kiss|darling|soulmate)\b/.test(t)) return 'love'
...
export default function App() {
  ...
  useEffect(() => {
    ...
    function drawFaerie(ctx2, f, ts) {
      ...
      if (gatherModeRef.current && f.radius < OR + 50) {
        ctx2.beginPath()
        ctx2.moveTo(fx, fy)
        ctx2.lineTo(OX + Math.cos(f.angle) * OR * 0.85, OY + Math.sin(f.angle) * OR * 0.85 * 0.7)
        ctx2.strokeStyle = f.col + Math.round(al * 0.15 * 255).toString(16).padStart(2,'0')
        ctx2.lineWidth = 0.7
        ctx2.stroke()
      }
    ...
    function updateOrbVisual() {
      if (!colorLockedRef.current) {
        orbOhRef.current  = hexLerp(orbOhRef.current,  tgtOhRef.current,  0.025)
        orbOcRef.current  = hexLerp(orbOcRef.current,  tgtOcRef.current,  0.025)
        orbOdRef.current  = hexLerp(orbOdRef.current,  tgtOdRef.current,  0.025)
        orbGs1Ref.current = rgbaLerp(orbGs1Ref.current, tgtGs1Ref.current, 0.025)
        orbGs2Ref.current = rgbaLerp(orbGs2Ref.current, tgtGs2Ref.current, 0.025)
      }
      orbEl.style.background = `radial-gradient(circle at 38% 33%, ${orbOhRef.current}, ${orbOcRef.current} 40%, ${orbOdRef.current} 100%)`
      orbEl.style.boxShadow  = `0 0 35px 10px ${orbGs1Ref.current}, 0 0 70px 28px ${orbGs2Ref.current}, inset 0 0 28px rgba(0,0,0,0.3)`
    }
    ...
    function loop(ts) {
      ...
      if (gatherModeRef.current) {
        gatherTRef.current = Math.min(gatherTRef.current + 0.003, 1)
  ...
  useEffect(() => {
    doRevealRef.current = (text) => {
      ...
      setTimeout(() => {
        ...
        setAnswer(text)
        setUiState('answer')
        setTimeout(() => setAnswerVis(true), 200)
      }, 600)
    }
  })
  ...
  return (
    <div style={{ minHeight:'100vh', background:'#07040f', display:'flex', flexDirection:'column', alignItems:'center', position:'relative', overflow:'hidden', fontFamily:"'Raleway',sans-serif", paddingTop:'2.5rem' }}>
      ...
      <div style={{ textAlign:'center', marginBottom:'2rem', position:'relative', zIndex:8 }}>
        <h1 style={{ fontFamily:"'Cinzel',serif", fontSize:25, fontWeight:400, color:'rgba(255,255,255,0.88)', letterSpacing:'0.08em', marginBottom:5 }}>
          The Oracle Orb
        </h1>
        <p style={{ fontSize:11, color:'rgba(255,255,255,0.28)', letterSpacing:'0.14em', fontStyle:'italic' }}>
          Ask with intention, and receive with an open heart
        </p>
      </div>

      <div ref={sceneRef} style={{ position:'relative', width:320, height:320, flexShrink:0, zIndex:4, marginBottom:'2rem', pointerEvents:'none' }}>
        <div style={{ position:'absolute', width:240, height:240, top:'50%', left:'50%', transform:'translate(-50%,-50%)', borderRadius:'50%', background:'radial-gradient(circle,rgba(120,90,255,0.09) 0%,transparent 70%)', animation:'softbreathe 5s ease-in-out infinite', pointerEvents:'none', zIndex:2 }} />
        <div ref={orbElRef} style={{ position:'absolute', width:180, height:180, top:'50%', left:'50%', transform:'translate(-50%,-50%)', borderRadius:'50%', zIndex:3 }} />
      </div>
      ...
      {uiState === 'question' && (
        <div style={{ position:'relative', zIndex:8, width:'100%', maxWidth:400, display:'flex', flexDirection:'column', alignItems:'center', gap:13, padding:'0 1.5rem' }}>
          <input
            ...
            style={{ width:'100%', background:'rgba(255,255,255,0.045)', border:'1px solid rgba(255,255,255,0.09)', borderRadius:10, padding:'13px 18px', color:'rgba(255,255,255,0.78)', fontFamily:"'Raleway',sans-serif", fontSize:14, outline:'none', textAlign:'center' }}
          />
      ...
      {uiState === 'answer' && (
        <div style={{ position:'relative', zIndex:8, width:'100%', maxWidth:400, display:'flex', flexDirection:'column', alignItems:'center', gap:13, padding:'0 1.5rem' }}>
          ...
          <p style={{ fontFamily:"'Cinzel',serif", fontSize:13.5, lineHeight:2, color:'rgba(255,255,255,0.82)', textAlign:'center', letterSpacing:'0.03em', opacity: answerVis ? 1 : 0, transition:'opacity 1.4s ease', maxWidth:360 }}>
            {answer}
          </p>
